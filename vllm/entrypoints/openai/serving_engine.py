...
class OpenAIServing:
    ...
    def create_error_response(
            ...
            error_context: Optional[dict] = None) -> ErrorResponse:
    ...
    def handle_abort_if_needed(
        ...
        """Check if request was aborted and return appropriate error
        response."""
        ...
        for output in request_output.outputs:
            if output.finish_reason == "abort":
                if request_output.error_context:
                    ...
                    err_type = (error_ctx.error_type.value if isinstance(
                        error_ctx.error_type, ErrorType) else
                                error_ctx.error_type)

                    # handle kv connector failures with 502 bad gateway
                    if err_type == ErrorType.KV_TRANSFER_FAILURE.value:
                        status_code = HTTPStatus.BAD_GATEWAY
                        # format rich error message with metadata
